...
public class ProgressGradeRenewalReaderConfig {
    ...
    public PagingQueryProvider progressTargetQueryProvider() {
        ...
        queryProvider
                .setFromClause("from (select mh.member_no, "
                + "mh.renewalGradeDate, "
                + "o.order_no "
                    + "from (select m.member_no, "
                    + "m.next_renewal_grade_date as renewalGradeDate, "
                    + "ifnull(max(h.grade_at), m.register_datetime) as lastRenewalDate "
                        + "from members m "
                        + "left outer join grade_histories h "
                        + "on h.member_no = m.member_no "
                        + "group by m.member_no) mh "
                    + "left outer join orders o "
                    + "on mh.member_no = o.member_no "
                    + "and o.order_datetime > mh.lastRenewalDate) a "
